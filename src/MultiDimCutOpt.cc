...
#include "../interface/MultiDimCutOpt.h"

using namespace std;
using namespace RooFit;

MultiDimCutOpt::MultiDimCutOpt():
  runAll(true),
  run2011(false),
  run2012(false),
  misIdType(1),
...
void MultiDimCutOpt::makeInitialDatasets(){

  w->factory("B_s0_MM[5000,5800]");
  w->var("B_s0_MM")->setBins(100); // IMPORTANT THIS
  ...
  if ( runAll || ( run2011 && run2012 ) ) {
    ...
    sig_mc_cut = "itype==-71 || itype==-81";
    bd_mc_cut  = "itype==-75 || itype==-85";
    phi_mc_cut = "itype==-76 || itype==-86";
...
RooPlot* MultiDimCutOpt::getSimplePlot(TString var, TString data, TString pdf){
...
void MultiDimCutOpt::fitAndSetConst(TString pdf, TString data) {

  if (!w->pdf(pdf) || !w->data(data)){
    cerr << "MultiDimCutOpt::fitAndSetConst() -- NOT VALID" << endl;
    exit(1);
  }
...
void MultiDimCutOpt::makePDFs(){

  w->factory("sig_mc_mean[5350,5450]");
  w->factory("CBShape::sig_mc_cb1( B_s0_MM, sig_mc_mean, sig_mc_sigma1[20,5,200], sig_mc_alpha_1[0.5,0.,5], sig_mc_n_1[0.5,0.,10] )");
  w->factory("CBShape::sig_mc_cb2( B_s0_MM, sig_mc_mean, sig_mc_sigma2[20,5,200], sig_mc_alpha_2[-0.5,-5.,0], sig_mc_n_2[0.5,0.,10] )");
  ...
  w->factory("CBShape::bd_mc_cb1( B_s0_MM, bd_mc_mean, bd_mc_sigma1[20,10,200], bd_mc_alpha_1[0.5,0.,5], bd_mc_n_1[0.5,0.,10] )");
  w->factory("CBShape::bd_mc_cb2( B_s0_MM, bd_mc_mean, bd_mc_sigma2[20,10,200], bd_mc_alpha_2[-0.5,-5.,0], bd_mc_n_2[0.5,0.,10] )");
  ...
  w->factory("CBShape::rho_mc_cb1( B_s0_MM, rho_mc_mean, rho_mc_sigma1[20,10,200], rho_mc_alpha1[0.5,0.,5.], rho_mc_n1[0.01,0.,40.])");
  w->factory("CBShape::rho_mc_cb2( B_s0_MM, rho_mc_mean, rho_mc_sigma2[20,10,200], rho_mc_alpha2[-0.5,-5.,0.], rho_mc_n2[0.01,0.,40.])");
  ...
  w->factory("CBShape::phi_mc_cb1( B_s0_MM, phi_mc_mean, phi_mc_sigma1[100,0,400], phi_mc_alpha1[0.5,0.,5.], phi_mc_n1[0.01,0.,100.])");
  w->factory("CBShape::phi_mc_cb2( B_s0_MM, phi_mc_mean, phi_mc_sigma2[100,0,400], phi_mc_alpha2[-0.5,-5.,0.], phi_mc_n2[0.1,0.,100.])");
  ...
  w->factory("CBShape::lb_mc_cb1( B_s0_MM, lb_mc_mean, lb_mc_sigma1[100,0,400], lb_mc_alpha1[0.5,0.,5.], lb_mc_n1[0.01,0.,100.])");
  w->factory("CBShape::lb_mc_cb2( B_s0_MM, lb_mc_mean, lb_mc_sigma2[100,0,400], lb_mc_alpha2[0.5,0.,5.], lb_mc_n2[0.1,0.,100.])");
  ...
  w->factory("ArgusBG::part_reco_pdf( B_s0_MM, part_reco_m0[5226], part_reco_c[-10,-100.,0.], part_reco_p[0.4,0.,1.] )");
  w->factory("Exponential::bkg_pdf( B_s0_MM, bkg_exp_p1[-0.02,-0.04,0.] )");
...
void MultiDimCutOpt::plotShapes(){

  TCanvas *canv = new TCanvas("shapes","shapes",1600,1200);
  canv->Divide(3,2);
  RooPlot* sig_mc_plot = getSimplePlot("B_s0_MM","full_sig_mc","sig_mc_pdf");
  RooPlot *bd_mc_plot  = getSimplePlot("B_s0_MM","full_bd_mc","bd_mc_pdf");
  RooPlot *rho_mc_plot = getSimplePlot("B_s0_MM","full_rho_mc","rho_mc_pdf");
  RooPlot *phi_mc_plot = getSimplePlot("B_s0_MM","full_phi_mc","phi_mc_pdf");
  RooPlot *lb_mc_plot = getSimplePlot("B_s0_MM","full_lb_mc","lb_mc_pdf");
...
void MultiDimCutOpt::plot(RooAbsData *data, RooAbsPdf *pdf, TString ext, double *cutvals){
...
double MultiDimCutOpt::fitAndReduce(const double *cutvs) {
...
double MultiDimCutOpt::fitAndReduce(TString cut){
...
RooDataHist* MultiDimCutOpt::reduce(TString cut_expr){

  n_reducs++;

  RooDataSet *full = (RooDataSet*)w->data("full_data");
  RooDataSet *reduced = (RooDataSet*)full->reduce(SelectVars(RooArgSet(*w->var("B_s0_MM"))),Cut(cut_expr.Data()),Name(Form("dset%d",n_reducs)));
  RooDataHist *binned = reduced->binnedClone(Form("dhist%d",n_reducs));
  delete reduced;
  return binned;
}

void MultiDimCutOpt::getEventEstimates(TTree *tree) {
...
TCut MultiDimCutOpt::getCut(double *cutvals) {
...
TCut MultiDimCutOpt::getBkgSel(){

  if ( runAll || ( run2011 && run2012 ) ) {
    return TCut("( (itype==71 || itype==81) && (B_s0_MM>5600) )");
  }
  if ( run2011 && !run2012 ) {
    return TCut("( (itype==71) && (B_s0_MM>5600) )");
  }
  if ( run2012 && !run2011 ) {
    return TCut("( (itype==81) && (B_s0_MM>5600) )");
  }
...
double MultiDimCutOpt::getFOM(double *cutvals) {
...
Scan1DResult MultiDimCutOpt::scan1D( int varInd, double lowVal, double highVal, int npoints, double *cutvals ){
  TGraph *gr = new TGraph();
...
Scan2DResult MultiDimCutOpt::scan2D( int varInd1, int varInd2, double lowVal1, double highVal1, int npoints1, double lowVal2, double highVal2, int npoints2, double *cutvals ) {
...
void MultiDimCutOpt::runSimple(int type) {
  ...
  //RooDataSet *data = (RooDataSet*)w->data("full_data")->reduce(SelectVars(RooArgSet(*w->var("B_s0_MM"))),Name("result_data_red"),Cut(Form("bdtoutput>%4.2f && min_kaon_PIDK>%5.3f && max_pion_PIDK<%5.3f",opts_vals[0],opts_vals[1],opts_vals[2])));
  RooDataSet *data = (RooDataSet*)w->data("full_data")->reduce(SelectVars(RooArgSet(*w->var("B_s0_MM"))),Name("result_data_red"),Cut(Form("bdtoutput>%4.2f && min_kaon_ProbNNk>%5.3f && min_pion_ProbNNpiKp>%5.3f",opts_vals[0],opts_vals[1],opts_vals[2])));
...
void MultiDimCutOpt::drawAtCut(TString name, double bdt_cut, double kaon_cut, double pion_cut) {

  if (pion_cut>0) pion_cut*=-1;

  //RooDataSet *data = (RooDataSet*)w->data("full_data")->reduce(SelectVars(RooArgSet(*w->var("B_s0_MM"))),Name(Form("%s_data_red",name.Data())),Cut(Form("bdtoutput>%4.2f && min_kaon_PIDK>%5.3f && max_pion_PIDK<%5.3f",bdt_cut,kaon_cut,pion_cut)));
  RooDataSet *data = (RooDataSet*)w->data("full_data")->reduce(SelectVars(RooArgSet(*w->var("B_s0_MM"))),Name(Form("%s_data_red",name.Data())),Cut(Form("bdtoutput>%4.2f && min_kaon_ProbNNk>%5.3f && min_pion_ProbNNpiKp>%5.3f",bdt_cut,kaon_cut,pion_cut)));
...
void MultiDimCutOpt::setAxes(TGraph *obj, TString xtitle, TString ytitle){
...
void MultiDimCutOpt::setAxes(TH1 *obj, TString xtitle, TString ytitle){
...
void MultiDimCutOpt::setAxes(TH1 *obj, TString xtitle, TString ytitle, TString ztitle){
