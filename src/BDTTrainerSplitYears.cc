...
#include "../interface/BDTTrainerSplitYears.h"

using namespace std;
using namespace TMVA;

BDTTrainerSplitYears::BDTTrainerSplitYears(TString _name):
	BaseAnalyser(_name),
	outfilename("MVATrainingOutputSplitYears.root"),
	evCount(0),
	numberOfBDTs(2),
  doBDTCycling(true)
{
...
BDTTrainerSplitYears::~BDTTrainerSplitYears(){}

void BDTTrainerSplitYears::Init(Looper *l){
	cout << Form("%-30s","BDTTrainerSplitYears::Init()") << " " << "Initialising Analyser (" << name << ")." << endl;
...
void BDTTrainerSplitYears::Term(Looper *l){

  reweightFile->Close();

	for (int b=0; b<numberOfBDTs; b++) {

		cout << Form("%-30s","BDTTrainerSplitYears::Term()") << " " << "Preparing training and test trees" << endl;
		factoryContainer["2011"][b]->PrepareTrainingAndTestTree("","!V");
		factoryContainer["2012"][b]->PrepareTrainingAndTestTree("","!V");

		// TMVA methods here
		cout << Form("%-30s","BDTTrainerSplitYears::Term()") << " " << "Booking TMVA methods" << endl;
		...
		cout << Form("%-30s","BDTTrainerSplitYears::Term()") << " " << "Train, test and evaluate TMVA methods" << endl;
	...
	cout << Form("%-30s","BDTTrainerSplitYears::Term()") << " " << "Training output written to: " << outFile->GetName() << endl;
  delete outFile;

	cout << Form("%-30s","BDTTrainerSplitYears::Term()") << " " << "Terminating Analyser (" << name << ")." << endl;
}

bool BDTTrainerSplitYears::AnalyseEvent(Looper *l){
