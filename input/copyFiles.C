...
void CopyDir(TDirectory *source) {
   ...
   TDirectory *adir = savdir->mkdir(TString(source->GetName()).ReplaceAll(".root",""));
   adir->cd();
...
void copyFiles() {
   ...
   TFile *f = new TFile("PIDHists.root","recreate");
   CopyFile("PIDHists_Strip20r1_MagDown.root");
   CopyFile("PIDHists_Strip20r1_MagUp.root");
   CopyFile("PIDHists_Strip20_MagDown.root");
   CopyFile("PIDHists_Strip20_MagUp.root");
   f->ls();
   delete f;
}
